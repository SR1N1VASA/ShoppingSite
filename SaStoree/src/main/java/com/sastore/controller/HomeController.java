...
import com.sastore.service.impl.UserSecurityService;
...
public class HomeController {
	...
	public String index(Model model, Principal principal) {
		...
		for(int i=1;i<=6 && i<bookList.size();i++)
			firstList.add(bookList.get(i));
		model.addAttribute("firstList",firstList);
		List<Electronic> electronicList = electronicService.findAll();
		List<Electronic> firstelectronicList = new ArrayList();
		for(int i=1;i<=6 && i<electronicList.size();i++)
			firstelectronicList.add(electronicList.get(i));
		model.addAttribute("firstelectronicList",firstelectronicList);
		List<Electronic> secondelectronicList = new ArrayList();
		for(int i=7;i<=12 && i<electronicList.size();i++)
			secondelectronicList.add(electronicList.get(i));
		model.addAttribute("secondelectronicList",secondelectronicList);
		return "index";
	}
	...
	@RequestMapping("/electronicshelf")
	public String electronicshelf(Model model, Principal principal) {
		if(principal != null) {
			String username = principal.getName();
			User user = userService.findByUsername(username);
			model.addAttribute("user", user);
		...
		model.addAttribute("activeAll",true);
		model.addAttribute("msg","Electronic Products");
	...
	@RequestMapping("/electronicDetail")
	public String electronicDetail(
			@PathParam("id") Long id, Model model, Principal principal
			) {
		if(principal != null) {
			String username = principal.getName();
